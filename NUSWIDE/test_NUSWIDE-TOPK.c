...
void evaluate_hashing_global_ball(hash_ensemble* hashed_database, hash_ensemble* hashed_queries, int n_database, int n_queries, int max_tables, groundtruth_matrix *groundtruth, char* filename_results, float min_r, float max_r, float* table_weights){
    ...
    for(i=0;i<n_queries;i++){
        ...
        for(t=0;t<9;t++){
            ...
            printf("DEN FOR PREC = %d\n",den_for_prec);
            float this_prec = ((float) this_hits_topK)/(((float) n_queries)*((float) den_for_prec));
            
            if(count_K_predicted>0)
                av_prec_topK[t] += this_prec;
            else
                av_prec_topK[t] += 1.0/((float) n_queries);

            printf("THIS PREC = %f, CUM=%f\n",this_prec,av_prec_topK[t]);
        }
...
int main(int argc, char *argv[])
{
    ...
    for(t=0;t<hashed_database->n_tables;t++){
        ...
        sprintf(table_file_name, "BoHash_NUSWIDE_Database_%dBits_Model%d.txt", nbits, t);
    ...
    for(t=0;t<hashed_queries->n_tables;t++){
        char table_file_name[100];
        sprintf(table_file_name, "BoHash_NUSWIDE_Queries_%dBits_Model%d.txt", nbits, t);
